...
db = firestore.client()
db.collection('menu').document('Epicuria').delete()

html_link = 'https://menu.dining.ucla.edu/Menus/Epicuria/today'
#grabbing the page
...
col = db.collection(u'menu').document('Epicuria').set({'name':'Epicuria'})
for food_block in foodlist:
    ...
    col = db.collection(u'menu').document(u'Epicuria').collection(time_periods[per_count]).document(main_title)
    if main_title == 'Dolci':
